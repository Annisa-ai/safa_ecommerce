...
export default function AdminContentPage() {
  ...
  function handleBgImageChange(file: File) {
    ...
    setHeroForm((prev: any) => ({ ...prev, backgroundImage: localPreviewUrl }))
  }
  ...
  const handleHeroSave = async () => {
    setIsUploading(true)
    try {
      let finalImageUrl = heroForm.backgroundImage

      // Jika ada file baru di dropzone, upload paksa ke Supabase Storage terlebih dahulu
      if (bgImageFile) {
        const fileExt = bgImageFile.name.split('.').pop()
        const fileName = `hero_${Date.now()}.${fileExt}`
        
        // Ganti 'konten' dengan nama bucket storage Supabase lu yang berstatus PUBLIC
        const { data: uploadData, error: uploadError } = await supabase.storage
          .from('konten') 
          .upload(`hero/${fileName}`, bgImageFile, {
            cacheControl: '3600',
            upsert: true
          })
        ...
        // Ambil Public URL permanen dari Supabase Storage
      ...
      // Gabungkan URL permanen ke dalam payload form sebelum disimpan ke database
      const updatedHeroForm = { ...heroForm, backgroundImage: finalImageUrl }
      
      await updateContent('hero', updatedHeroForm)
      setBgImageFile(null) // Reset file state karena sudah ter-upload
      setSaved(true)
      setTimeout(() => setSaved(false), 2000)
    } catch (error) {
      console.error('Gagal menyimpan konten hero:', error)
      alert('Gagal mengunggah gambar atau menyimpan data.')
    } finally {
          ...
          <div className="space-y-4">
            ...
            {saved && (
              <div className="p-3 bg-green-100 border border-green-300 rounded-lg text-green-800 text-sm font-medium">
                ✓ Perubahan berhasil disimpan ke cloud storage
              </div>
            )}
            ...
            >
              {isUploading ? 'Mengunggah Gambar...' : 'Simpan Perubahan'}
            </Button>
          </div>
        </Card>
      )}
          ...
          <div className="space-y-4">
            <div className="p-4 bg-muted/20 rounded-lg">
              ...
              <p className="text-sm text-foreground/70">
                <span className="font-medium">Proyek-proyek Selesai:</span> {aboutContent?.projects}
              </p>
            </div>
            <div className="p-3 bg-blue-100 border border-blue-300 rounded-lg text-blue-800 text-sm">
